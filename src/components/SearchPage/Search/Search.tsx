...
const Search:any = () => {
  ...
  useEffect(() => {
   startFilter();
   //각자 가져오기 
  }, [enterKeyPressed, filterStartDate, filterEventDuration, filterEventDuration, departmentStoreCategory, dateSelected]);
  ...
  const startFilter = () => {
    ...
    // 지역 필터
    if (enterKeyPressed === true || datePicker.length !== 0 || filterEventDuration.length !== 0 || filterEventDuration.length !== 0 || departmentStoreCategory.length !== 0) {
  ...
  return (
    <SearchContainer>
      <SearchContainerTop>
        <SearchInput type="text" value={searchTerm}placeholder="키워드를 입력해주세요. (예. 서울, 액세서리)" onChange={(event) => setSearchTerm(event.target.value)} onKeyPress={checkKeypress}/>
        <CalendarContainer>
          {/* 팝업스토어 시작 날짜 선택 */}
          <DatePickerContainer 
